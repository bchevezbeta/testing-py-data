...
class VisionApi:
    ...
    def detect_text_intl(self, input_filenames, num_retries=3, max_results=6):
        """Uses the Vision API to detect text in the given file.
        """
        images = {}
        for filename in input_filenames:
...
class Index:
    ...
    def lookup_all(self):
		...
		docs_with_word = self.redis_token_client.keys('*')
		hits = set([
    ...
    def print_lookup_all(self):
        ...
        output = list()
        data = list()
        hits = self.lookup_all()
        if not hits:
            output.append('No keys')
        for i in hits:
        	data.append(self.print_lookup(i[0]))
        	output.append(i[0])
	return output, data
    ...
    def print_lookup(self, *words):
        ...
        for i in hits:
			text = i[0].replace("/Users/bchevez/Desktop/Otis_Test_Data/batch/intl_png/", "https://storage.googleapis.com/dynamic-charter-7017/")
			text = text.replace("/Users/bchevez/Desktop/Otis_Test_Data/batch/png/", "https://storage.googleapis.com/dynamic-charter-7017/")
			output.append(text)
	return output
...
def extract_descriptions(input_filename, index, texts):
    ...
    if texts:
        ...
        #os.system('gsutil cp -r %s gs://dynamic-charter-7017' % input_filename)
        print('File %s has been processed' % input_filename)
...
# [START get_text]
def get_text_from_files(vision, index, input_filenames, lang):
    """Call the Vision API on a file and index the results."""
    if (lang =='jp'):
    	texts = vision.detect_text_intl(input_filenames)
    else:
...
def main(input_dir,lang):
    ...
    for filenames in batch(fileslist):
        get_text_from_files(vision, index, filenames, lang)
